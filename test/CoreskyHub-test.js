const { deploy, gasCalculate, getDeployed, getDeployedInstance, getAttach } = require("./deploy.js");
...
const { structHash, signHash, signTypedData, signProjectVote, signDeployApNFT, signDeposit } = require("./signCoreskyHub.js");
...
const groupID = 1;
if (step == 1 || all) {
  describe("CoreskyHub-test-eth-Fundraising-【success】", function () {
    let lpid = 256;
    let lpid2 = 100;
    let presaleUsers = [];
    let accounts;
   ...
   */
      ...
      let tx = await coreskyHub.connect(bob).createAllocation(groupID, _roundID, _target, _payment, _nftPrice, _startTime, _endTime, _voteEndTime, _mintEndTime, _totalQuantity);
   ...
   */
      ...
      let tx = coreskyHub.connect(bob).createAllocation(_groupID, _roundID, _target, _payment, _nftPrice, _startTime, _endTime, _voteEndTime, _mintEndTime, _totalQuantity);
...
if (step == 2 || all) {
  describe("CoreskyHub-test-token-Fundraising-【success】", function () {
    let lpid = 256;
    let lpid2 = 100;
    let accounts;
    let metaTxLib;
   ...
   */
      ...
      let signature = await walletSign(sam, "addProposal",metaTxLib.address, message);
   ...
   */
      ...
      console.log("coreskyHub.isProject", await coreskyHub.isProject(project.address));
    });

    it("coreskyHub-applyProjectVote-sam【Reset after time expires】", async function () {
      const { admin, operator, bob, sam, user } = await signer();
      ...
      let botSignature =  await signProjectVote(serialNo, projectVote.projectAddr, supportCount, opposeCount, voteRatio, expireTime, deadline);
      let tx = coreskyHub.connect(project).applyProjectVote(signature, projectVote, botSignature);
      await (0, chai.expect)(tx).to.be.revertedWith( "ResetAfterTimeExpires");
      // await (0, chai.expect)(tx).to.be.revertedWith("Reset after time expires");
    });
   ...
   */
      ...
      console.log("preSale param:", roundID, preSaleID, preSaleNum, owner, "allowace:", allowaceBefore, allowaceAfter);
      let tx = await launchpad.connect(user).preSale(roundID, preSaleID, preSaleNum, voteNum, {
        from: owner,
        value: 0,
      });
   ...
   */
      ...
      // await (0, chai.expect)(tx).to.be.revertedWith("Mint not started");
      await (0, chai.expect)(tx).to.be.revertedWith( "MinNotStarted");
      ...
      await coreskyHub.connect(user).apNftMint(signature, lpid);
   ...
   */
      ...
      // get allowance
      let allowaceAfter = await erc20token.allowance(owner, launchpad.address);
      console.log("preSale-limitPreSale param:", roundID, preSaleID, preSaleNum, owner, "allowace:", allowaceBefore, allowaceAfter);
      let tx = launchpad.connect(user).preSale(roundID, preSaleID, preSaleNum, voteNum, {
        from: owner,
        value: 0,
      });
   ...
   */
      ...
      console.log("paused-PreSale param:", roundID, preSaleID, preSaleNum, owner, "allowace:", allowaceBefore, allowaceAfter);
      let tx = launchpad.connect(user).preSale(roundID, preSaleID, preSaleNum, voteNum, {
        from: owner,
        value: 0,
      });
   ...
   */
      ...
      // get allowance
      let allowaceAfter = await erc20token.allowance(owner, launchpad.address);
      console.log("unpause-PreSale param:", roundID, preSaleID, preSaleNum, owner, "allowace:", allowaceBefore, allowaceAfter);
      let tx = await launchpad.connect(user).preSale(roundID, preSaleID, preSaleNum, voteNum, {
        from: owner,
        value: 0,
      });
   ...
   */
      ...
      // get allowance
      let allowaceAfter = await erc20token.allowance(owner, launchpad.address);
      console.log("SoldOut-PreSale param:", roundID, preSaleID, preSaleNum, owner, "allowace:", allowaceBefore, allowaceAfter);
      let tx = launchpad.connect(user).preSale(roundID, preSaleID, preSaleNum, voteNum, {
        from: owner,
        value: 0,
      });
    ...
    it("coreskyHub-refundFundraisingVote-【fundraisingStatus=1】-success-【sendFundraising】", async function () {
      ...
      let total = await launchpad.getWithdrawableAmount(erc20token.address);
      if (total > 0) {
        let tiems = parseInt(total / amount);
        for (let i = 0; i < tiems; i++) {
          serialNo += i;
          ...
          // console.log("Fundraising thisBalance After: ", n(await launchpad.thisBalance()));
          console.log("Fundraising getWithdrawableAmount After: ", await launchpad.getWithdrawableAmount(erc20token.address));
          console.log("operator Balance:", operator.address, await erc20token.balances(operator.address));
        }
      }
...
if (step == 3 || all) {
  describe("CoreskyHub-test-eth-Fundraising-【fail】", function () {
    let lpid = 256;
    let lpid2 = 100;
    let accounts;
    let metaTxLib;
   ...
   */
    ...
    it("coreskyHub-refundFundraisingVote-user", async function () {
      ...
      let voteCount = await launchpad.getVoteNum(lpid, user.address);
      ...
      // await (0, chai.expect)(tx).to.be.revertedWith( "AllocationDoesNotExist");

      // 设置预售结束时间-已结束
      await coreskyHub.connect(operator).setEndTime(roundID, parseInt(new Date().getTime() / 1000) - 10);
...
if (step == 4 || all) {
  describe("CoreskyHub-test-token-Fundraising-【fail】", function () {
    let lpid = 256;
    let lpid2 = 100;
    let accounts;
    let metaTxLib;
   ...
   */
    ...
    //   it("coreskyHub-setFundraisingStatus-【fundraisingStatus=1】", async function () {
    //     const { admin, operator, bob, sam, user } = await signer();
    //     /**
    //     function setFundraisingStatus(uint256 _roundID, uint8 _fundraisingStatus) public onlyRole(OPERATOR_ROLE) {
    //     )
    //  */
    //     console.log("getFundraisingStatus before:", await launchpad.getFundraisingStatus(lpid));
    //     await coreskyHub.connect(operator).setFundraisingStatus(lpid, 1);
    //     console.log("getFundraisingStatus  after:", await launchpad.getFundraisingStatus(lpid));
    //   });
